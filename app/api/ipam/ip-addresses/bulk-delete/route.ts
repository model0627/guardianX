...
/**
 * @swagger
 * /api/ipam/ip-addresses/bulk-delete:
 *   post:
 *     summary: 여러 IP 주소 일괄 삭제
 *     tags: [IPAM]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (ip_address_ids.length > 100) {
      return NextResponse.json(
        { error: '한 번에 최대 100개의 IP 주소만 삭제할 수 있습니다.' },
        { status: 400 }
      );
    }
    ...
    const ipAddressCheck = await query(`
      ...
      WHERE ip.id = ANY($1::uuid[]) AND ir.tenant_id = $2 AND ip.is_active = true
    `, [ip_address_ids, tenantId]);

    if (ipAddressCheck.rows.length === 0) {
      return NextResponse.json(
        { error: '삭제할 수 있는 IP 주소가 없습니다.' },
        { status: 404 }
      );
    }
    ...
    console.log(`[API] Bulk deleted ${deletedCount} IP addresses by user ${user.userId}`);
    ...
    return NextResponse.json({
      ...
      message: `${deletedCount}개의 IP 주소가 성공적으로 삭제되었습니다.${notFoundCount > 0 ? ` (${notFoundCount}개는 찾을 수 없음)` : ''}`
    });

  } catch (error) {
    console.error('Bulk IP deletion error:', error);
    ...
    return NextResponse.json(
      { error: `IP 주소 삭제에 실패했습니다: ${error instanceof Error ? error.message : 'Unknown error'}` },
      { status: 500 }
    );
  }
}