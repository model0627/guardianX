...
const DATABASE_FIELDS = [
  { key: 'name', label: '라이브러리 이름 (name)', required: true },
  { key: 'version', label: '버전 (version)', required: false },
  { key: 'vendor', label: '제조사 (vendor)', required: false },
  { key: 'type', label: '라이브러리 타입 (library_type)', required: false },
  ...
  { key: 'device_name', label: '디바이스명 (device_name)', required: false },
  { key: 'host_name', label: '호스트명 (host_name)', required: false },
  { key: 'status', label: '상태 (status)', required: false },
];

export default function FieldMappingModal({ isOpen, onClose, apiConnection, onSave }: FieldMappingModalProps) {
  ...
  const fetchSampleData = async () => {
    ...
    setApiFields([]); // 기존 필드 초기화
    setSampleData(null);
    
    try {
      let response;
      
      // Google Sheets인 경우 별도 처리
      if (apiConnection.connection_type === 'google_sheets') {
        // Google Sheets 연결 정보 확인
        const sheetsInfoResponse = await fetch(`/api/api-connections/${apiConnection.id}/sheets-info`, {
          credentials: 'include'
        });
        
        if (!sheetsInfoResponse.ok) {
          console.error('Failed to fetch Google Sheets info');
          return;
        }
        
        const sheetsInfo = await sheetsInfoResponse.json();
        
        if (sheetsInfo.auth_type === 'oauth' && sheetsInfo.spreadsheet_id) {
          // OAuth 방식 - 비공개 스프레드시트
          response = await fetch('/api/google-sheets/private', {
            ...
            credentials: 'include',
            body: JSON.stringify({
              spreadsheetId: sheetsInfo.spreadsheet_id,
          ...
          // 공개 스프레드시트
          response = await fetch('/api/google-sheets/fetch', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            console.log('API Fields:', Object.keys(sampleItem));
            setSampleData(sampleItem);
            setApiFields(Object.keys(sampleItem));
          } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div>
            <h2 className="text-xl font-semibold">필드 매핑 설정 - {apiConnection.name}</h2>
            <p className="text-sm text-gray-600 mt-1">API 응답 필드를 라이브러리 데이터베이스 필드에 매핑하세요.</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {DATABASE_FIELDS.map((dbField) => (
                  <div key={dbField.key} className="flex items-center space-x-3">
                    <div className="flex-1">
                      ...
                      >
                        <option value="">
                          {fetchingData ? 'API 데이터를 불러오는 중...' : 
                           apiFields.length === 0 ? '새로고침 버튼을 눌러주세요' : '선택하세요...'}
                        </option>
                        {apiFields.map((field) => (
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  <RefreshCw className={`w-4 h-4 mr-1 ${fetchingData ? 'animate-spin' : ''}`} />
                  {fetchingData ? '로딩 중...' : '새로고침'}
                </button>
              </div>
